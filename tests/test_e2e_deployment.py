...
def docker_image():
    ...
    # Build
    subprocess.run(["docker", "build", "-t", IMAGE_NAME, "."], check=True, capture_output=True)
...
def test_container_safe_crash_missing_env(docker_image):
    ...
    Container should exit gracefully (code 1) when env vars are missing,
    ...
    assert (
        "validation error" in result.stderr.lower() or "validation error" in result.stdout.lower()
    )
