"""
tests/redate/test_adapter_openai.py
...
from redate.domain_models import KnowledgeExtractionResult, RetrievalContext, WeeklyReport
...
async def test_generate_embedding_siliconflow(respx_mock):
    ...
    route = respx_mock.post("https://api.siliconflow.cn/v1/embeddings").mock(
        return_value=Response(
            200,
            json={"data": [{"embedding": [0.1, 0.2, 0.3], "index": 0, "object": "embedding"}]},
        )
    )
...
@pytest.mark.asyncio
async def test_generate_period_report_success(respx_mock):
    """Test successful generation of a weekly report."""
    expected_report = {
        "start_date": "2026-01-01",
        "end_date": "2026-01-07",
        "summary_text": "Weekly Review",
        "key_events": ["Event 1"],
    }
    ...
    respx_mock.post("https://openrouter.ai/api/v1/chat/completions").mock(
        return_value=Response(
            200,
            json={"choices": [{"message": {"content": None, "parsed": expected_report}}]},
        )
    )

    adapter = OpenAIAdapter()
    context = RetrievalContext(
        vector_results=["v"], keyword_matches=["k"], knowledge_graph_summary=["g"]
    )
    report = await adapter.generate_period_report(context, date(2026, 1, 1), date(2026, 1, 7))

    assert isinstance(report, WeeklyReport)
    assert report.summary_text == "Weekly Review"
...
async def test_generate_period_report_error_handling(respx_mock):
    ...
    adapter = OpenAIAdapter()
    # Mock context
    context = RetrievalContext(vector_results=[], keyword_matches=[], knowledge_graph_summary=[])

    with pytest.raises(Exception, match="Internal Server Error"):
        await adapter.generate_period_report(context, date(2026, 1, 1), date(2026, 1, 7))
